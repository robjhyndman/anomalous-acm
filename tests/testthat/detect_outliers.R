context("anomaly tests")

test_that("Time-series is constructed correctly, features are extracted properly and that anomaly method returns expected number of outliers.", {
  z <- ts(matrix(rnorm(3000),ncol=100),freq=4)
  expect_that(ncol(z) = 100, is_true())
  y <- tsmeasures(z)
  expect_that(ncol(y) = 18, is_true())
